...
public class TowerAI : MonoBehaviour
{
    ...
    private void targetPoint()  //finds where/how to point to look at target and points that way
    {
        ...
        nozzle.transform.rotation = Quaternion.Lerp(nozzle.transform.rotation, output, tower.getTurnSpeed());
    }
    ...
    private IEnumerator firing()    //Coroutine for creating the bullets as long as there's a target
    {
        ...
        while (target != null && target.active)     //Will keep firing on one target until that target is dead
        {
            ...
            boolet.SendMessage("Mult", tower.getDamageMult());  //And how much damage it does

            yield return new WaitForSeconds(tower.getFireRate());   //For some reason the tower won't turn until this is done. Maybe something to do with it being a Coroutine?
        }
